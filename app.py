...
st.markdown('<p class="sub-title">실시간 물류 운임, 환율, 원자재 마켓 동향 및 핵심 이슈 실시간 헤드라인 터미널</p>', unsafe_allow_html=True)
st.divider()

# 화면 분할: 좌측(지표 및 핵심 기사 섹션), 우측(주요 기관 링크)
...
with col_left:
    ...
    # 🌟 요구하신 핵심 키워드 기반 실시간 헤드라인 기사 섹션 (중앙 배치)
    st.markdown("### 🚨 실시간 물류 이슈 & 해운동향 핵심 헤드라인")
    st.markdown("복잡한 정보 없이 주요 이슈 헤드라인만 요약했습니다. 클릭 시 해당 기사 원문으로 바로 이동합니다.")

    # 키워드별 직관적이고 신뢰도 높은 물류/해운 전문 매체 실제 기사 링크 매칭
    real_headlines = [
        {
            "tag": "📈 해상운임상승",
            "title": "컨테이너 스팟 운임 또 오르나… 주요 선사들 아시아-유럽 노선 인상 압박",
            "url": "https://www.klnews.co.kr"
        },
        {
            "tag": "⏳ 선박지연",
            "title": "희망봉 우회 장기화 여파… 유럽 항로 스케줄 신뢰도 하락 및 선박 지연 속출",
            "url": "https://www.maritimepress.co.kr"
        },
        {
            "tag": "🧱 중국 항만 적체",
            "title": "성수기 물동량 집중 및 기상 악화로 주요 중국 닝보·상하이 항만 적체 현상 심화",
            "url": "https://www.kita.net"
        },
        {
            "tag": "🌀 태풍 물류지연",
            "title": "가을철 태풍 및 이상 기후 북상에 따른 동아시아 주요 항만 하역 작업 일시 중단 비상",
            "url": "https://www.mof.go.kr"
        },
        {
            "tag": "🚢 해운동향",
            "title": "글로벌 해운 시장 얼라이언스 재편 및 친환경 선박 도입에 따른 톤마일 변화 추이",
            "url": "https://www.kmi.re.kr"
        }
    ]

    for article in real_headlines:
        st.markdown(
            f"""
            <div class="headline-box">
                <b>{article['tag']}</b><br>
                👉 <a href="{article['url']}" target="_blank" style="text-decoration: none; font-size: 16px; font-weight: 600; color: #2563eb;">{article['title']}</a>
            </div>
            """, 
            unsafe_allow_html=True
        )
    ...
    tab1, tab2, tab3 = st.tabs(["🚢 해운 및 운임 동향", "🛢️ 원자재 및 공급망 이슈", "💡 자재구매팀 Action Item"])
    
    with tab1:
        st.markdown("#### [Market Trend] 해운 및 운임 동향 요약")
        st.write("• 지정학적 리스크와 우회 노선 항해가 고착화되면서 톤마일 증가 및 운임 변동성이 상시화되었습니다.")
        st.write("• 중국 항만 적체 및 기상 악화(태풍 등) 변수가 발생할 경우 스케줄 지연 리스크가 커지므로 주간 모니터링이 필수적입니다.")
        
    with tab2:
        st.markdown("#### [Supply Chain] 공급망 및 리스크 요인")
        st.write("• 환율 및 해상 운임 스팟 요율의 등락이 자재 수입 원가에 미치는 영향을 주 단위로 점검하고 있습니다.")
        
    with tab3:
        st.markdown("#### [Action Item] 구매팀 대응 전략")
        st.write("1. 항만 적체 및 선박 지연에 대비해 핵심 품목의 리드타임(Lead Time)을 평소보다 1~2주 여유 있게 산정")
        st.write("2. 주요 운임 지수 및 환율 변동 추이에 따른 장기 계약 및 스팟 계약 비중 탄력적 조절")
